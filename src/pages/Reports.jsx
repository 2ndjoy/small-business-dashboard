      ...
      <table className="min-w-full border-collapse border border-gray-300">
        <thead>
          <tr className="bg-yellow-100 text-yellow-700">
            ...
            <th className="border border-gray-300 px-4 py-2 text-right">Total Orders</th>
        ...
        <tbody>
          {customers.map(({ id, name, email, orders, lastOrder }) => (
            <tr key={id} className="hover:bg-yellow-50">
              ...
              <td className="border border-gray-300 px-4 py-2 text-right">{orders}</td>
              <td className="border border-gray-300 px-4 py-2">{lastOrder}</td>
            </tr>
          ))}
        </tbody>
      </table>
